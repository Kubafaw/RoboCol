...
func _get_next_point(_current_pos : Vector2) -> Vector2: # global position
	...
	if x_movement_prior:
		for x in range(1, path_max_lenght+1):
			var tile_occupancy : Node2D = Gamedata._Main_Map._get_tile_occupancy(_current_pos + x_vector * x)
		...
		for y in range(1, path_max_lenght+1):
			var tile_occupancy : Node2D = Gamedata._Main_Map._get_tile_occupancy(_current_pos + y_vector * y)
			if !tile_occupancy == null:
				if next_nav_point == Vector2(0, 0):
